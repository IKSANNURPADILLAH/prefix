#!/bin/bash
# ================================
# AUTOPROXY SETUP SCRIPT (Netplan Edition)
...
NETPLAN_FILE="/etc/netplan/99-multi-ip.yaml"
NETMASKS="24"
...
# === KONFIGURASI IP PERMANEN MENGGUNAKAN NETPLAN ===
echo "[+] Mengatur IP statik ke Netplan: $NETPLAN_FILE"

# Ambil IP utama interface
MAIN_IP=$(ip -4 addr show "$INTERFACE" | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | head -n1)

# Ambil gateway
GATEWAY=$(ip route | grep default | awk '{print $3}')
if [ -z "$GATEWAY" ]; then
    echo "[!] Gateway tidak ditemukan secara otomatis. Silakan isi secara manual." >&2
    exit 1
fi

# Tulis konfigurasi awal Netplan
...
# Tambahkan semua IP ke Netplan
for i in $(seq $START $END); do
    IP="$IP_PREFIX.$i"
    echo "        - $IP/24" | sudo tee -a "$NETPLAN_FILE" > /dev/null
done

# Tambahkan gateway & DNS
